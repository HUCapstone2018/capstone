 ...
 * @access public
 */
    public $uses = array('Nodes.Node', 'Taxonomy.Vocabulary', 'Seo', 'Menus.Link');
    ...
    public function admin_index() {
        ...
        if(!empty($this->data)) {
            ...
            $this->Session->setFlash(__('SEO/OM Options have been saved', true, array('class' => 'success')));
    ...
    function sitemap() {
        ...
        $nodes = $this->Node->find('all',array('conditions' => array('Node.status =' => 1,'Node.type'=>'blog')));
...
*/
    ...
    function sitemapxml() {
        ...
        $this->Node->contain('Seo');
        $menuData 	= $this->Link->find('all',array('conditions' => array('Link.menu_id =' => 3,'Link.status'=>1)));
        $sitemapData = $this->__getSiteMapData($this->Node->find('all',array('conditions' => array('Node.status =' => 1, 'Node.type'=>'blog'))));

        $this->set(compact('sitemapData','menuData'));
