<?php
echo $this->Html->script('/wysiwyg/js/wysiwyg', array('inline' => false));
?>
	...
	<div class="row-fluid">
		<div class="span12 actions">
			...
			<?php
				...
				parse_str( parse_url( $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], PHP_URL_QUERY ), $c );
				
				$controler = !empty($this->request->params['named']['controler']) ? 
									$this->request->params['named']['controler'] 
									: (
											!empty($c['controler']) ? 
											$c['controler'] 
											: 
				...
				echo $this->Croogo->adminAction(
					...
					array('action' => 'add', 'editor' => 2, 'controler'=> $controler)
				);
			?>
			</ul>
		</div>
	...
	<?php
		...
		foreach ($attachments as $attachment):
			...
			$actions[] = $this->Croogo->adminRowAction('',
				array('controller' => 'attachments', 'action' => 'edit', $attachment['Attachment']['id'], 'editor' => 2),
			...
			$insertCode = $this->Html->link('', '#', array(
				'onclick' => "Croogo.Wysiwyg.custom('" . $attachment['Attachment']['slug'] . "');",
				'icon' => 'paper-clip',
