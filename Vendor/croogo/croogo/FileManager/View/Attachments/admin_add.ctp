...
if (isset($this->request->params['named']['editor'])) {
	$formUrl['editor'] = $this->request->params['named']['editor'];
}
if (isset($this->request->params['named']['controler'])) {
	$formUrl['controler'] = $this->request->params['named']['controler'];
}
...
if(!empty($referrer))
	echo $this->Form->input('referrer_link', array('type'=>'hidden','value'=>$referrer));
