 ...
 */
	protected function _saveUploadedFile($data) {
		...
		$slugged_name = Inflector::slug($fileTitle, '-'). $extension;
		$destination = WWW_ROOT . $this->uploadsDir . DS . $slugged_name;
		
		if (file_exists($destination)) {
			$newFileName = time() . '-' . $slugged_name;
			$destination = WWW_ROOT . $this->uploadsDir . DS . $newFileName;
		} else {
			$newFileName = $slugged_name;
		}
