	...
	public function admin_add() {
		...
		if ($this->request->is('post') || !empty($this->request->data)) {
			...
			if ($this->Attachment->save($this->request->data)) {
				...
				if (isset($this->request->params['named']['editor'])) {
					$kess_emmak = $this->request->params['named']['editor'];
					$controler  = !empty($this->request->params['named']['controler']) ? $this->request->params['named']['controler'] : "";
					switch($kess_emmak){
						case 1:
							if(!empty($this->request->data['Attachment']['referrer_link']))
								$this->redirect($this->request->data['Attachment']['referrer_link']);
							else
						...
						case 2:
							if(empty($controler))
								$this->redirect(array('action' => 'custom'));
							else
								$this->redirect(array('action' => 'custom', 'controler'=>$controler));
